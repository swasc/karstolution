...
def constants(TC, pCO2):
    ...
    k1p = 10**(329.850 - 110.54 * np.log10(TK) - 17265.4/TK)
    ...
    K5 = 1.707e-4
    K6 = 10**(-356.3094 + 21834.37 / TK - 0.060919964 * TK + 126.8339 * np.log10(TK) - 1684915 / (TK**2))   
    ...
    KW = 10**(22.801 - 4787.3 / TK - 0.010365 * TK - 7.1321 * np.log10(TK))
    ...
    for i in range(1,11):
        ...
        gammaH = 10**(-A * np.sqrt(IS) / (1 + B * 9 * np.sqrt(IS)))       #Nach Dreybrodt1988/S15
        gammaCa = 10**(-A * 4 * np.sqrt(IS) / (1 + B * 5 * np.sqrt(IS)))  #Nach Dreybrodt1988/S15
        gammaHCO = 10**(-A * np.sqrt(IS) / (1 + B * 5.4 * np.sqrt(IS)))   #Nach Dreybrodt1988/S15
        gammaOH = 10**(-A * np.sqrt(IS) / (1 + B * 3.5 * np.sqrt(IS)))
        gammaCO3 = 10**(-A * 4 * np.sqrt(IS) / (1 + B * 5.4 * np.sqrt(IS)))
    ...
    ac = np.array([gammaCa, gammaCO3, gammaH, gammaHCO, gammaOH])
    rc = np.array([k1m, k1p, k2m, k2p, km, kp, kHCO])
    cc = np.array([Ca, H, HCO, OH, CO2])
    mp = np.array([C1, C2])
    ks = np.array([K0, K1, K2, KH])
